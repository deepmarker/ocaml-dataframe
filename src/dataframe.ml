...
let iteri : type a. (int -> a -> 'b) -> a t -> unit =
 fun f t ->
  let n = length t in
  for i = 0 to n - 1 do
    f i (row t i)
...
let rev_iter : type a. (a -> 'b) -> a t -> unit =
 fun f t ->
  let n = length t in
...
let rev_iteri : type a. (int -> a -> 'b) -> a t -> unit =
 fun f t ->
  let n = length t in
  for i = n - 1 downto 0 do
    f i (row t i)
...
let fold_left : type a. ('b -> a -> 'b) -> 'b -> a t -> 'b =
 fun f a t ->
  let n = length t in
  let acc = ref a in
  for i = 0 to n - 1 do
...
let fold_right : type a. (a -> 'b -> 'b) -> a t -> 'b -> 'b =
 fun f t a ->
  let n = length t in
  let acc = ref a in
  for i = n - 1 downto 0 do
...
let rev_map : type a. (a -> 'b) -> a t -> 'b list =
 fun f t -> fold_left (fun a v -> f v :: a) [] t

let map : type a. (a -> 'b) -> a t -> 'b list =
 fun f t -> fold_right (fun v a -> f v :: a) t []

let rev_mapi : type a. (int -> a -> 'b) -> a t -> 'b list =
 fun f t ->
  let _, r = fold_left (fun (i, a) v -> (succ i, f i v :: a)) (0, []) t in
  r

let mapi : type a. (int -> a -> 'b) -> a t -> 'b list =
 fun f t ->
  let _, r =
    fold_right (fun v (i, a) -> (pred i, f i v :: a)) t (pred (length t), [])
  in
  r
...
module L = struct
  ...
  let t5 a b c d e =
    t5 (of_list a) (of_list b) (of_list c) (of_list d) (of_list e)

  let t6 a b c d e f =
    t6 (of_list a) (of_list b) (of_list c) (of_list d) (of_list e) (of_list f)

  let t7 a b c d e f g =
    t7 (of_list a) (of_list b) (of_list c) (of_list d) (of_list e) (of_list f)
      (of_list g)

  let t8 a b c d e f g h =
    t8 (of_list a) (of_list b) (of_list c) (of_list d) (of_list e) (of_list f)
      (of_list g) (of_list h)

  let t9 a b c d e f g h i =
    t9 (of_list a) (of_list b) (of_list c) (of_list d) (of_list e) (of_list f)
      (of_list g) (of_list h) (of_list i)

  let t10 a b c d e f g h i j =
    t10 (of_list a) (of_list b) (of_list c) (of_list d) (of_list e) (of_list f)
      (of_list g) (of_list h) (of_list i) (of_list j)
end
